...
import { Loader2, ArrowUpRight, ArrowDownRight, PlayCircle, Eye, MessageCircle, Share2, CircleDollarSign, Minus, Heart } from "lucide-react";
...
export function TikTokAccountOverview() {
  ...
  if (isLoading) return null; // Yanındaki kartlar loader göstereceği için bura boş durabilir
    ...
    <Card className="w-full h-full flex flex-col border-border/40 shadow-sm bg-card">
      <CardHeader className="pb-4">
        <CardTitle className="text-sm font-medium text-muted-foreground flex justify-between">
          <span>7-Day Performance Metrics</span>
        </CardTitle>
      </CardHeader>
      
      <CardContent className="flex-1 flex flex-col">
        {/* 6 metrik, 2 satır 3 sütun halinde kartın içini doldurur */}
